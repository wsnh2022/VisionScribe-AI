...
Tesseract is OCR Pipeline only. Not required if using Direct Vision exclusively.
...
| `notion-client` | v2.0-beta Notion push only |

Tesseract OCR must be installed separately (Windows installer): https://github.com/UB-Mannheim/tesseract/wiki
...
Override path in `config.ini [Paths] TesseractExe` if installed elsewhere.
